...
class WhatIfForm(forms.Form):
    ...
    live_confirmation_candles = forms.MultipleChoiceField(
        label="Live Candles Before Confirmation",
        choices=[(i, i) for i in range(1, 12)],
        widget=forms.CheckboxSelectMultiple,
        initial=[1],
    )
    reversed_confirmation_candles = forms.MultipleChoiceField(
        label="Reversed Candles Before Confirmation",
        choices=[(i, i) for i in range(1, 12)],
        widget=forms.CheckboxSelectMultiple,
        initial=[1, 2],
    )
    ...
    live_tp = forms.FloatField(label="Live Take Profit (%)", initial=2)
    reversed_sl = forms.FloatField(label="Reversed Stop Loss (%)", initial=1)
    reversed_tp = forms.FloatField(label="Reversed Take Profit (%)", initial=2)
    ...
    entry_hours = forms.MultipleChoiceField(
        label="Entry Hours",
    ...
    entry_week_days = forms.MultipleChoiceField(
        label="Entry Week Days",
        choices=FORM_DAY_CHOICES,
        widget=forms.CheckboxSelectMultiple,
        required=False,
        initial=[2, 3, 4, 5, 6, 7, 1],
    )
    liquidation_hours = forms.MultipleChoiceField(
        label="Liquidation Hours",
        choices=[(i, i) for i in range(0, 24)],
        widget=forms.CheckboxSelectMultiple,
        required=False,
        initial=[3, 4, 5, 14, 15, 16, 17],
    )
    liquidation_week_days = forms.MultipleChoiceField(
        label="Liquidation Week Days",
        choices=FORM_DAY_CHOICES,
        widget=forms.CheckboxSelectMultiple,
        required=False,
        initial=[2, 3, 4, 5, 6],
...
class WhatIfRSIForm(forms.Form):
    use_log_scale = forms.BooleanField(label="Use Log Scale", required=False)
...
class WhatIfPerHourForm(forms.Form):
    ...
    start_date_gte = forms.DateField(
        label="Start Date (gte)", required=False, initial="2025-05-01"
    )
    start_date_lt = forms.DateField(label="Start Date (lt)", required=False)
    week_days = forms.MultipleChoiceField(
        label="Week Days",
