...
class Command(BaseCommand):
        ...
        **filter_kwargs,
        ...
        for hour in range(24):
            ...
            for position in positions:
                ...
                ) = self.calculate_position_outcome(
                    symbol, position, tp, sl, until_date
                )
    ...
    def handle(self, *args, **options):
        ...
        print(f"Generating {symbol} algorithm input data for {till_date}")
        ...
        # write csv header
        with open(
            f"{settings.ALGORITHM_EXPORT_PATH}/algorithm_input-{symbol}-{till_date}-{strategy_type}.csv",
            "w",
            newline="",
        ) as f:
            writer = csv.writer(f)
