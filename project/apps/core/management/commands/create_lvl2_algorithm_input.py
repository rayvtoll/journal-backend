            ...
            for position in hour_positions:
                ...
                if ohlcv_s.exists():
                    ...
                    position.amount = round(
                        (total_returns) / sl / position.entry_price * weight,
                        4,
                    )
                ...
                for candle in ohlcv_s:
                    ...
                    if position.side == "SHORT":
                        ...
                        if candle.low <= position.entry_price - (
                            ...
                            total_returns += position.what_if_returns
                            break
            hour_row["performance"] = round(total_returns - INITIAL_CAPITAL, 2)
            performance_list.append(hour_row)
        df = pd.DataFrame(performance_list)
        till_date_df = pd.read_csv(
            f"data/algorithm_input-{symbol}-{till_date}-reversed.csv"
        )
        till_date_df = pd.merge(till_date_df, df[["hour", "performance"]], on="hour")
        till_date_df["trade_lvl2"] = till_date_df.apply(
            lambda row: (row.trade and row.performance > 0),
            axis=1,
        )
        print("level 2 algorithm input:")
        print(till_date_df)
        till_date_df.to_csv(
            f"data/algorithm_input-{symbol}-{till_date}-reversed-lvl2.csv", index=False
        )
